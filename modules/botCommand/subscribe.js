...
module.exports = async (ctx) => {
  ...
  let branches = update.message.text.replace('/subscribe', '')
      ...
      for (let i = 0; i < branches.length; i++) {
        ...
        if (branchDetails.length === 2) {
            ...
            if (project) {
              ...
              await db.Subscribe.add(db_chat, branch)
  ...
  if (addedBranch.length) {
    await ctx.reply(update.message.from.first_name + ' Вы подписались такие ветки:\n ' + addedBranch.join('\n '))
